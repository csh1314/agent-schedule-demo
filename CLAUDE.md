...
**当用户的输入满足以下任一条件时，必须立即启动 Pipeline 调度流程，不得先做其他事情（如探索代码、提问确认等）：**

1. 描述了一个新功能或功能变更（如"新增XX功能"、"添加XX"、"给XX加上XX"、"实现XX"）
2. 描述了一个 bug 修复或优化需求（如"修复XX"、"优化XX"、"改进XX"）
3. 描述了一个完整的项目需求（如"做一个XX应用"、"开发XX系统"）
4. 用户明确说"走 pipeline"、"按流程来"、"启动调度"

**不触发的情况**：纯粹的提问（如"这个项目是做什么的？"）、配置修改请求（如"修改 CLAUDE.md"）、git 操作请求（如"提交代码"）。
